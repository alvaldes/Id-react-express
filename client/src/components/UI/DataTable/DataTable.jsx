...
//NOTA: aqui se definen el nombre de las celdas de la bd, se especifican propiedades 
// const headCells1 = [
//   {
//     id: 'id',
//     numeric: false,
...
function DataTable(props) {
  ...
  const [orderBy, setOrderBy] = React.useState(props.headCells[0].id);
  const [page, setPage] = React.useState(0);
  const [dense, setDense] = React.useState(false);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const {rows}= props;
//esto es para cambiar el orden de los datos
  ...
  const handleSelectAllClick = (event) => {
    if (event.checked) {
      // console.log(rows);// aqui quitar es para ver q esta mostrando rows, es un arreglo
      // const newSelecteds = rows.map((n) => n.id);
      props.onStoreSelected(rows);
      return;
    }
  ...
  const handleClick = (event, id) => {
    ...
    props.onStoreSelected(newSelected);
  };
//paginas de la tabla
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
  };
//cambiar los datos 10 x pagina, siguiente pagina mostrar siguientes 10 datos
